...
- Inspect existing scripts with `rg`, `sed`, and `git diff`.
- Edit with `apply_patch`.
- Verify with `bash -n`, ShellCheck when available, and targeted script runs through `exec_command`.
- Keep this skill functionally aligned with `.claude/skills/bash-pro`.
